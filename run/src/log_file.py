...
class LogFile():
    def __init__(self, experiment_path, observation_config, agent_config, train_test_config, train_test_config_name, overwrite, fps, resume_training, log_every):
        ...
        ## 아래 resume 때문에 수정된 부분 (헷갈리면 원본 코드 볼 것)
        if self.train:
            if os.path.exists(experiment_path) and not overwrite and not resume_training:
                print(f'{experiment_path} already exits. ')
                raise Exception(
                    'Experiment name already exists. If you want to overwrite, use flag -ow')
            if os.path.exists(experiment_path) and not resume_training:
    ...
    def create_folders(self):
        if self.train:
            ...
            os.makedirs(f'{self.experiment_path}/logs', exist_ok=True)
            os.makedirs(f'{self.experiment_path}/logs/training', exist_ok=True)
            os.makedirs(f'{self.experiment_path}/logs/training/plots', exist_ok=True)
            os.makedirs(f'{self.experiment_path}/weights', exist_ok=True)
            os.makedirs(f'{self.experiment_path}/monitor', exist_ok=True)
            os.makedirs(f'{self.experiment_path}/configs', exist_ok=True)
            os.makedirs(f'{self.experiment_path}/episode_stats', exist_ok=True)
            os.makedirs(f'{self.experiment_path}/weights/optimizers', exist_ok=True)
        else:
    ...
    def save_config_files(self, observation_config, agent_config, train_test_config):
        ...
        # in rlfold folder.
        env_config_path = f"{os.getenv('RLFOLD_ROOT')}/config/envs/{env_config_name}"
