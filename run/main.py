...
def main():
    ...
    experiment_path = f'{os.getenv("HOME")}/results/rlfold/{experiment_name}'
    ...
    observation_config_path = f"{RLFOLD_ROOT}/config/experiments/{experiment_name}/observation.yaml"
    agent_config_path = f"{RLFOLD_ROOT}/config/experiments/{experiment_name}/agent.yaml"
    train_test_config_path = f"{RLFOLD_ROOT}/config/experiments/{experiment_name}/{train_test_config_name}"
    ...
    env_config_path = f"{RLFOLD_ROOT}/config/envs/{env_config_name}"
    ...
    # if train:
    #     wandb.init(project='rlfold', name=experiment_name, config=agent_config, dir=experiment_path)
    ...
    for ep in range(n_episodes):
        ...
        while not done:
            ...
            # 10000 스텝마다 주기적으로 모델 저장 (0 스텝 제외)
            if steps > 0 and steps % 10000 == 0:
            	agent.save_periodic_models(step=steps)
